...
const EngineSelection = struct {
    ...
    enable_memory_latticedb: bool = false,
    ...
    fn enableAllOptional(self: *EngineSelection) void {
        ...
        self.enable_memory_latticedb = true;
    }
...
fn parseEnginesOption(raw: []const u8) !EngineSelection {
    ...
    while (it.next()) |token_raw| {
        ...
        } else if (std.mem.eql(u8, token, "latticedb")) {
            selection.enable_memory_latticedb = true;
        } else {
...
pub fn build(b: *std.Build) void {
    ...
    const engines_raw = b.option(
        []const u8,
        "engines",
        "Memory engines list. Tokens: base|minimal|all|none|markdown|memory|api|sqlite|lucid|redis|lancedb|postgres|clickhouse|latticedb (default: base,sqlite)",
    );
    ...
    const enable_memory_latticedb = engines.enable_memory_latticedb;
    ...
    build_options.addOption(bool, "enable_memory_latticedb", enable_memory_latticedb);
    ...
    const lib_mod: ?*std.Build.Module = if (is_wasi) null else blk: {
        ...
        if (enable_memory_latticedb) {
            const lattice_dep = b.dependency("latticedb", .{
                .target = target,
                .optimize = optimize,
            });
            module.addImport("lattice", lattice_dep.module("lattice"));
        }
        break :blk module;
    };
